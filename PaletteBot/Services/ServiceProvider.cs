...
// Credits for this code mostly goes to Kwoth, since FairPlay137 can't code it himself apparently

namespace PaletteBot.Services
{
    public interface IPaletteServiceProvider : IServiceProvider, IEnumerable<object>
    {
        T GetService<T>();
    }
    public class PaletteServiceProvider : IPaletteServiceProvider
    {
        public class ServiceProviderBuilder
        {
            ...
            public PaletteServiceProvider Build()
            {
                return new PaletteServiceProvider(_dict);
            }
            ...
            public ServiceProviderBuilder LoadFrom(Assembly assembly)
            {
                ...
                var services = new Queue<Type>(allTypes
                        .Where(x => x.GetInterfaces().Contains(typeof(IPaletteBotService))
                ...
                while (services.Count > 0)
                {
                    ...
                    if (swInstance.Elapsed.TotalSeconds > 5)
                        _log.Warn($"{type.Name} took {swInstance.Elapsed.TotalSeconds:F2}s to load.");
        ...
        private PaletteServiceProvider() { }

        public PaletteServiceProvider(IDictionary<Type, object> services)
        {
